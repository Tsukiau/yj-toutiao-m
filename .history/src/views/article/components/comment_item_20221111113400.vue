  ...
  <van-cell class="comment-item">
    ...
    <div slot="label">
      ...
      <div class="bottom-info">
        <span class="comment-pubdate">{{comment_item.pubdate | relativeTime}}</span>
        <van-button
          class="reply-btn"
          round
        >回复 {{comment_item.reply_count}}</van-button>
      </div>
    </div> 
  </van-cell>
</template>
...
<script>
import { addLike,deleteCommentLike} from '@/api/comment'
export default {
  name: 'CommentItem',
  components: {},
  props: {
  ...
  methods: {
    async onLike(){
      ...
      try {
        ...
        // 执行点赞
         await addLike(this.comment_item.com_id)
         this.comment_item.like_count++
       }
       this.comment_item.is_liking = !this.$childrencomment_item.is_liking
      }catch(err){
          console.log(err)
          this.$toast('操作失败')
